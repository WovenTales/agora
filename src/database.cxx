...
/*! \param ref the column name to access
 */
const Database::Table::Column &Database::Table::operator [](const std::string &ref) const {
	//! \todo Implement DEBUG level in log
	Log << "Accessing column '" << ref << "' in table '" << title << "'" << Log.ENDL;
	return columns.at(ref);
}
...
std::string Database::getTitle() const {
	Data out = getColumns({ meta["title"] })[0];
	return out[meta["title"]];
}
 ...
 *
 *  \todo Convert output to allow positional indices rather than forcing use of Table::Column
 *
...
Database::DataList Database::getColumns(const std::initializer_list<Database::Table::Column> &cols,
	...
	for (pair<const Table::Column, string> w : where) {
		...
		// Only add tables if we're pulling all columns and the table isn't already in the list
		if ((cols.size() == 0) && (find(tables.begin(), tables.end(), t) == tables.end())) {
			tables.push_back(t);
		}
	...
	Log << "Completed" << (Log.ENDL | Log.CONTINUE) << "Executing command...";
	//! \todo Implement DEBUG level in log
	Log << "'" << cmd << "'";
	Log << (Log.ENDL | Log.CONTINUE);
